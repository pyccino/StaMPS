...
{\sffamily\Huge{\textbf{\textsc{\\}}}}
  ...
  {\sffamily\Huge{\textbf{{\\
   StaMPS/MTI  Manual\\
     ...
     \vspace{5mm}
     {\sffamily\Large{\textbf{Version 1.0.0 (Windows port, preview)}}}
\vspace{5mm}
...
Windows users: see Section~\ref{sec:windows} for Windows-specific setup and limitations.
...
\section{Windows native support (SNAP path)}
\label{sec:windows}

Starting with release v1.0.0 of the Windows port fork
(\url{https://github.com/pyccino/StaMPS}), StaMPS runs natively
on Windows 10/11 for the SNAP preprocessor path. See \texttt{INSTALL.md}
in the repository for step-by-step setup. Known limitations:

\begin{itemize}
  \item \texttt{dismph} (X11/Motif viewer) is Linux/macOS only.
  \item DORIS, ROI\_PAC, GAMMA, ISCE preprocessor paths remain Linux-only.
  \item TRAIN atmospheric corrections are not ported to Windows.
\end{itemize}
